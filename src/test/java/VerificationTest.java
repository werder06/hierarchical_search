...
public class VerificationTest  {
  ...
  public void test() throws IOException, SolrServerException {
    ConcurrentUpdateSolrServer server = new ConcurrentUpdateSolrServer("http://localhost:8983/solr", 4, 2);
    ...
    checkQueryEquals(server, "first","parent_1","parent_2");

    checkQueryEquals(server, "third","parent_1" );

  }

   private void checkQueryEquals(ConcurrentUpdateSolrServer server, String request, String... expected) throws SolrServerException {
    checkQuery(server, "q={!parent which=type:parent}search_field:"+request, null,  expected);
    checkQuery(server, "search_field:" + request, "{!collapse field=parentId}", expected);
    checkQuery(server, "q={!join from=parentId to=id}search_field:"+request, null, expected);
   }

    private void checkQuery(ConcurrentUpdateSolrServer server, String q, String fq,String... expected) throws SolrServerException {
   ...
   private void checkResult(SolrDocumentList solrDocumentList, String... expected) {
      Assert.assertEquals(expected.length,solrDocumentList.size());
      checkResult(Iterables.transform(solrDocumentList, new Function<SolrDocument, String>() {
    ...
    public static void checkResult(Iterable<String> actualIds, String... expectedIds) {
        Assert.assertEquals("Actual values " + Iterables.toString(actualIds), expectedIds.length, Iterables.size(actualIds));
        for (String expected : expectedIds) {
            Assert.assertTrue("Expected " + expected + ", but not found in " + Iterables.toString(actualIds), Iterables.contains(actualIds, expected));
        }
        for (String actualId : actualIds) {
            Assert.assertTrue("Expected " + actualId + ", but not found in " + Arrays.toString(expectedIds), Iterables.contains(Arrays.asList(expectedIds), actualId));
        }
    }

  private SolrInputDocument createChild(String id, String parentId, String text, String parentData) {
